...
for index, row in df.iterrows():
    ...
    for a in ageGroup:
        for s in sex:
            ...
            os.system(f"python worldranking.py --type {type} --discipline {discipline} --sex {s} --ageCategory {a} --year {year} --regionType area --region \"south america\" --output tmp/leads/{a}/area/sa/{s}-{a}-{discipline}.csv")
